...
function keepTimeAndCalculateWPM() {
  if (startTime) {
    ...
    let accuracy =
        Math.floor(100 - wrongCharacters.length / characters.length * 100, 2);
    wpm.innerText = `wpm: ${cpm} \t accuracy: ${accuracy}% \t wc: ${
        typedCorrectText.innerText.split(" ").length}/ ${text.length}`;
    document.querySelector("article").scrollTop =
        typedWrongText.offsetTop - 500;
  }
}